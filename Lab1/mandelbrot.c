...
// Compiled only if LOADBALANCE = 0
	...
	parameters->begin_h = args->id * parameters->height / (float)NB_THREADS;
	parameters->end_h = (args->id + 1) * parameters->height / (float)NB_THREADS;
...
// Compiled only if LOADBALANCE = 1
	...
	// Number of chunks in x-, y-axis
	const int x_res = 10;
	const int y_res = 10;

	const float chunk_y_size = parameters->height / (float)y_res;;
	const float chunk_x_size = parameters->width / (float)x_res;

	for (int i = args->id; i < x_res * y_res; i += NB_THREADS)
	{

		float y = i / y_res;
		float x = i % x_res;

		parameters->begin_h = y * chunk_y_size;
		parameters->end_h = (y + 1) * chunk_y_size;
		parameters->begin_w = x * chunk_x_size;
		parameters->end_w = (x + 1) * chunk_x_size;
...
// Compiled only if LOADBALANCE = 2
	...
	while (atomic_count < ((x_res * y_res) - 1))
	{
		...
		float y = atomic_count / y_res;
		float x = atomic_count % x_res;
